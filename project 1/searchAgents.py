...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        "*** YOUR CODE HERE ***"
        eaten = state[1]
        return eaten[0] and eaten[1] and eaten[2] and eaten[3]
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            for i, corner in enumerate(self.corners):
                if (nextx, nexty) == corner:
                    newEaten[i] = True
            successors.append((((nextx, nexty), tuple(newEaten)), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(len(corners)):
        if state[1][i] is False:
            sum = max(sum, util.manhattanDistance(state[0], corners[i]))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    return 0 if len(food_list) == 0 else max([mazeDistance(food, position, problem.startingGameState) for food in food_list])
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        for food in food.asList():
            prob = PositionSearchProblem(
                gameState, start=startPosition, goal=food, warn=False, visualize=False)
            action = search.bfs(prob)
            if len(action) < cost:
                cost = len(action)
                bestAction = action
        return bestAction
